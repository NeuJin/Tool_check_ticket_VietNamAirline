...
class VNADirectAPI(FlightAPI):
    ...
    x_d_token: str = ""
    _xd_token_expiry: float = 0.0
    _xd_refresh_lock = threading.Lock()
    _xd_bg_running: bool = False
    ...
    @classmethod
    def _auto_fetch_xd_token(cls, callback=None) -> bool:
        """Tự động lấy x-d-token — dùng Edge thật sự để bypass bot detection."""
        if not cls._xd_refresh_lock.acquire(blocking=False):
            return False
        cls._xd_bg_running = True
        try:
            try:
                from playwright.sync_api import sync_playwright
            except ImportError:
                if callback:
                    callback(False, "Cần cài playwright:\npip install playwright\nplaywright install chromium")
                return False
            try:
                captured = []
                all_urls = []
                ...
                with sync_playwright() as p:
                    # Ưu tiên dùng Edge thật sự (pre-installed, không bị detect)
                    try:
                    ...
                    def on_request(req):
                        all_urls.append(req.url[:80])
                        tok = req.headers.get("x-d-token", "")
                        if tok and tok not in captured:
                            captured.append(tok)

                    page.on("request", on_request)

                    # Bước 1: thử nhiều URL vì booking.vietnamairlines.com/ trả 404
                    BOOKING_URLS = [
                        "https://booking.vietnamairlines.com/vi-VN/",
                        "https://booking.vietnamairlines.com/en-VN/",
                    ...
                    for burl in BOOKING_URLS:
                        try:
                            resp = page.goto(burl, timeout=20000, wait_until="commit")
                            nav_status = resp.status if resp else 0
                            if nav_status not in (404, 0):
                                break
                        except Exception:
                            continue

                    # Bước 2: chờ Angular/JS bundle thật sự load xong
                    # SPA load xong khi có nhiều hơn 10 requests và DOM ổn định
                    for _ in range(12):
                    ...
                    # Bước 3: trigger search từ JS context (nếu SPA đã load)
                    if scripts_loaded > 0:
                        page.evaluate(f"""
                        () => {{
                            const url = 'https://api-des.vietnamairlines.com/v2/search/air-bounds';
                            const body = JSON.stringify({{
                                commercialFareFamilies:['WEB'],
                                itineraries:[{{
                                    departureDateTime:'{search_date}T00:00:00.000',
                ...
                if captured:
                    cls.x_d_token = captured[-1]
                    cls._xd_token_expiry = time.time() + 1500
                    cls.reset_token()
                    if callback:
                        callback(True, captured[-1])
                    return True
                else:
                    vna_urls = [u for u in all_urls if "vietnamairlines" in u]
                    debug = (f"HTTP {nav_status} | URL: {final_url[:55]} | "
                             f"Scripts: {scripts_loaded} | "
                             f"Req: {len(all_urls)} (VNA:{len(vna_urls)})")
                    if callback:
            ...
            cls._xd_bg_running = False
            cls._xd_refresh_lock.release()
    ...
    def _get_token(cls) -> Optional[str]:
        # Tự động làm mới x_d_token trong background nếu đã hết hạn
        if not cls.x_d_token or time.time() >= cls._xd_token_expiry:
            if not cls._xd_bg_running:
                threading.Thread(
                    target=cls._auto_fetch_xd_token, daemon=True
                ).start()
...
class App:
    ...
    def _build_settings_tab(self, nb):
        ...
        api_f = ttk.LabelFrame(f, text="Vietnam Airlines Direct API", padding=15)
        api_f.pack(fill=tk.X)

        ttk.Label(api_f, text="x-d-token:").grid(
            row=0, column=0, sticky=tk.W, pady=4, padx=(4, 8))
        self._xd_token = tk.StringVar(value=self.settings.get("vna_xd_token", ""))
        xd_row = ttk.Frame(api_f)
        xd_row.grid(row=0, column=1, sticky=tk.EW)
        ttk.Entry(xd_row, textvariable=self._xd_token, width=48).pack(
            side=tk.LEFT, fill=tk.X, expand=True)
        self._fetch_btn = ttk.Button(xd_row, text="Tự động lấy",
                                     command=self._auto_fetch_xd_ui)
        self._fetch_btn.pack(side=tk.LEFT, padx=(6, 0))

        # Progress bar — hiện khi đang fetch token
        self._xd_progress = ttk.Progressbar(
            api_f, mode="indeterminate", length=300)
        self._xd_progress.grid(
            row=1, column=0, columnspan=2, sticky=tk.W, padx=(4, 0), pady=(4, 2))
        self._xd_progress.grid_remove()  # ẩn mặc định

        self._xd_status_var = tk.StringVar(value="")
        ttk.Label(api_f, textvariable=self._xd_status_var,
                  foreground="#2e7d32", justify=tk.LEFT, wraplength=500).grid(
            row=2, column=0, columnspan=2, sticky=tk.W, padx=(4, 0), pady=(0, 6))

        br = ttk.Frame(api_f)
        br.grid(row=3, column=0, columnspan=2, sticky=tk.W, pady=(4, 0))
        ...
        notes = (
            "• Nhấn 'Tự động lấy' để lấy x-d-token qua browser ẩn (~15 giây). Token hợp lệ ~25 phút.\n"
            "• Token tự động làm mới khi hết hạn mỗi khi bạn tìm kiếm chuyến bay.\n"
            "• Giá hiển thị là mức giá thấp nhất trong ngày, CHƯA bao gồm thuế/phí phụ.\n"
            "• Nên xác nhận giá cuối trên website chính thức vietnamairlines.com."
        )
    ...
    def _apply_api(self):
        tok = self.settings.get("vna_xd_token", "")
        VNADirectAPI.x_d_token = tok
        if tok and time.time() >= VNADirectAPI._xd_token_expiry:
            VNADirectAPI._xd_token_expiry = time.time() + 1500
    ...
    def _auto_fetch_xd_ui(self):
        """Chạy background thread lấy x-d-token, hiện progress bar."""
        self._fetch_btn.config(state="disabled")
        self._xd_progress.grid()          # hiện progress bar
        self._xd_progress.start(12)       # bắt đầu chạy
        self._xd_status_var.set("Đang mở browser để lấy token... (~30 giây)")

        def callback(success, data):
            def update():
                self._xd_progress.stop()
                self._xd_progress.grid_remove()   # ẩn lại
                self._fetch_btn.config(state="normal")
                if success:
                    self._xd_token.set(data)
                    self.settings["vna_xd_token"] = data
                    self.settings.save()
                    VNADirectAPI.x_d_token = data
                    VNADirectAPI._xd_token_expiry = time.time() + 1500
                    self._xd_status_var.set("✔ Token hợp lệ (~25 phút) — tự động làm mới khi hết hạn")
                else:
                    self._xd_status_var.set(f"Lỗi: {data}")
            self.root.after(0, update)

        threading.Thread(
            target=VNADirectAPI._auto_fetch_xd_token,
            args=(callback,),
            daemon=True,
        ).start()
